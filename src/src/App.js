...
export default function App() {
  ...
  const handleLogin = () => {
    ...
    if ((staff.includes(user) && pass === '1234') || (user === 'ADMIN888' && pass === 'HAUS2026')) {
      localStorage.setItem('haus_session', user);
      setIsLoggedIn(true);
    } else { alert('USER หรือ PASSWORD ไม่ถูกต้อง'); }
  };
    ...
    return (
      <div style={{ textAlign: 'center', padding: '60px 20px', backgroundColor: '#fff', minHeight: '100vh', fontFamily: 'sans-serif' }}>
        ...
        <div style={{ maxWidth: '300px', margin: '0 auto' }}>
          <input placeholder="USER" value={user} onChange={(e) => setUser(e.target.value.toUpperCase())} style={{ width: '100%', padding: '15px', marginBottom: '15px', borderRadius: '10px', border: '1px solid #000', textAlign: 'center', fontSize: '18px' }} />
          <input type="password" placeholder="PASSWORD" value={pass} onChange={(e) => setPass(e.target.value.toUpperCase())} style={{ width: '100%', padding: '15px', marginBottom: '25px', borderRadius: '10px', border: '1px solid #000', textAlign: 'center', fontSize: '18px' }} />
  ...
  return (
    <div style={{ textAlign: 'center', padding: '50px', fontFamily: 'sans-serif' }}>
      ...
      <button onClick={() => { localStorage.removeItem('haus_session'); setIsLoggedIn(false); }}>LOGOUT</button>
    </div>
  );
}
